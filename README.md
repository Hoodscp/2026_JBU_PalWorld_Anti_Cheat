...
| **Track 2 / P2** | MidHook 트램펄린 (조건분기 강제 + 명령어 주입) | 토글 시 즉시 적용 | 게임 전역 분기/누적 메커니즘 |
...
- `Menu` — 7탭 ImGui 오버레이 메뉴 (60+ 토글, 10+ 입력 필드)
...
| 기능 | 방식 | 설명 |
|------|------|------|
| 무한 기술 포인트 | Track 1 | TechnologyPoint = 9999 |
| 무한 보스 기술 포인트 | Track 1 | BossTechnologyPoint = 9999 |
| 연구 진행도 가속 | Track 2 / P2 | `addss xmm6, [rdi+0x14]` 직후 `mulss xmm6, xmm6` 주입 → 진행도 제곱 누적 |
...
| 기능 | 방식 | 설명 |
|------|------|------|
| 팰 무적 | Track 1 | 선택 슬롯 HP = 1,000,000,000 |
| 팰 최대 정신력 | Track 1 | Sanity = 100.0 |
| 팰 무한 MP | Track 1 | MP = 1,000,000,000 |
| 팰 최대 탤런트 | Track 1 | 4종 IV 모두 100 (이미 잡힌 팰) |
| 팰 레벨 강제 설정 | Track 1 | Level 1–255 |
| 팰 경험치 강제 설정 | Track 1 | Exp ≥ 0 |
| 새 팰 IV 100 강제 | Track 2 / P2 | `mov r14d, 0x65` 직전 `mov edi, 0x64` 주입 → 새로 생성/포획 펄의 4종 IV 100 |
| 보스 펄 포획 가능 | Track 2 / P2 | `[rax+0x598]` 보스 차단 플래그 0 강제 → 보스/머천트도 페일 스피어 포획 가능 |

> Track 1 토글은 팰 박스 컨테이너 한정 (Party/Otomo 미지원).
> Track 2 / P2 토글은 게임 전역에서 분기 조작 (슬롯 선택 무관).

#### Environment 탭
| 기능 | 방식 | 설명 |
|------|------|------|
| 항상 적정 온도 | Track 2 (MidHook) | Temperature 상태값 0(Normal) 강제 |
| 어디든 건축 가능 | Track 2 / P2 | `cmp [rcx+0xE4], sil` 직전에 같은 위치에 sil 강제 write → ZF=1 통과, 건축 가용성 검사 우회 |
...
**P2 — MidHook bodyEnabled 주입형** (원본 명령어는 보존, 직전/직후에 강제 명령어 삽입)

| 훅 이름 | 카테고리 | 주입 동작 | patchLen |
|---------|----------|-----------|----------|
| BuildAnywhereHook | 조건분기 강제 | `cmp [rcx+0xE4], sil` 직전에 `mov [rcx+0xE4], sil` 주입 → ZF=1 강제 | 7 bytes |
| CanCatchBossHook | 조건분기 강제 | `cmp byte [rax+0x598], 0` 직전에 `mov byte [rax+0x598], 0` 주입 | 14 bytes |
| PalStatsCreationHook | 레지스터 강제값 | `mov r14d, 0x65` 직전에 `mov edi, 0x64` 주입 → IV 100 | 6 bytes |
| ResearchIncreaseHook | 명령어 추가 | `addss xmm6, [rdi+0x14]` 직후에 `mulss xmm6, xmm6` 주입 → 제곱 누적 | 5 bytes |
